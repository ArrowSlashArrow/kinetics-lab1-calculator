...
def process_data(df):
    ...
    st.markdown(r"To find concentration, we can use the formula $A = abc$. We know $A$, $a$ and $b$, so we can find $c$ with $c = \frac{A}{ab}$. Doing this for each data point yields the following data:")
    ...
    cols = st.columns(3)
    fns = [lambda x: x, lambda x: [math.log(n) for n in x], lambda x: [1/n for n in x]]
    ...
    if most_linear_r2 < 0.9:
        st.markdown("Note: r^2^ is less than 0.9 for the most linear plot. Data may be noisy.")
    ...
    st.markdown("Assuming we have a large excess of bleach, we can determine that the concentration of bleach remains relatively constant to red40.")
    st.markdown(f"This means that we can simplify the rate law to $r=k'\\cdot[red40]^{most_linear}$, where k' is the pseudo-rate constant whose value is equal to $k\\cdot[bleach]^m_0 = k\\cdot{bleach_const}$")
...
def main():
    ...
    bleach_conc0 = st.number_input("Initial concentration of bleach in molar:", min_value=0
    )
    bleach_order = st.number_input("Order of reaction in respect to bleach (if given): ", value=1)
